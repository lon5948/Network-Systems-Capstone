...
def send_response(request_frame, client_socket, directory):
    ...
    if request_path == "/":
        ...
        for i in range(3):
            ...
            d_payload += "<br/>"
        d_payload += "</body></html>"
        d_frame = request_length.to_bytes(3, byteorder='big') + int(0).to_bytes(1, byteorder='big') + int(1).to_bytes(1, byteorder='big') + stream_id.to_bytes(4, byteorder='big') + d_payload.encode()

        h_payload = f"200 OK\r\nContent-Type:text/html\r\nContent-Length:{len(d_payload)}"
        h_frame = request_length.to_bytes(3, byteorder='big') + int(1).to_bytes(1, byteorder='big') + int(1).to_bytes(1, byteorder='big') + stream_id.to_bytes(4, byteorder='big') + h_payload.encode()
        ...
        with open(full_path, "rb") as file:
            ...
            while complete == 0:
                ...
                if not d_payload_next:
                    complete = 1
                d_frame = request_length.to_bytes(3, byteorder='big') + int(0).to_bytes(1, byteorder='big') + complete.to_bytes(1, byteorder='big') + stream_id.to_bytes(4, byteorder='big') + d_payload
                client_socket.send(d_frame)
    else:
